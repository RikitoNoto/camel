...
class Camel<T, C> {
  ...
  Future<int> send(C to, Message message) async{
    ...
    return await communicator.send(CommunicateData<T>(connection: connection, message: message));
  }
  ...
  Stream<CommunicateData<T>> listen(C bind) async*{
    await for(CommunicateData<T> data in await communicator.listen(bind)){
      ...
      if(command != null) command.execute(Uint8List.fromList(utf8.encode(data.message.body)));
      yield data;
    }
  }
}
